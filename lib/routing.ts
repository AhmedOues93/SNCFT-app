...
const fareForLine = (fares: FareInfo[], lineCode: JourneySegment['lineCode']): FareInfo | undefined =>
  fares.find((item) => item.lineCode === lineCode);
      ...
      trip.stops.forEach((departureStop, departureIndex) => {
        for (let arrivalIndex = departureIndex + 1; arrivalIndex < trip.stops.length; arrivalIndex += 1) {
          ...
          const segment: JourneySegment = {
            ...
            fareAmount: fareForLine(fares, trip.lineCode)?.amount,
            fareCurrency: fareForLine(fares, trip.lineCode)?.currency ?? 'TND',
          };
...
export const findJourneys = ({
  ...
  const byDeparture = buildPossibleSegments(trips, lineFilter, fares);
